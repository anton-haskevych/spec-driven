...
export function statusValues(markdown: string): string[] {
  return fieldValues(markdown, "status");
}

export function fieldValues(markdown: string, field: string): string[] {
  const section = new RegExp(`^${field}\\b`, "i");
  const values = outlineMarkdown(markdown)
    .codeSpans.filter((span) => section.test(span.section) && VALUE_TOKEN.test(span.code))
    .map((span) => span.code);
  return [...new Set(values)];
}

export function projectFieldValues(projectDir: string, field: string): string[] {
  return fieldValues(readTextIfExists(join(projectDir, PROJECT_TAXONOMY)) ?? "", field);
}
