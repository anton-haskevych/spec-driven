...
Once Stage B context is loaded, read [execute.md](execute.md) and follow it from §1 (its §0 entry section is for direct `/spec execute` invocations — context is already loaded here). The execution loop owns the chunk from this point: it opens with wave-based phase-seam recon (`/explore-waves`, `phase-exec` lens) to lock the seam and estimate testing issues, then TDD per unit, commit per logical change, mini-progress-update per chunk, full `/spec handoff` before the context budget runs out.
